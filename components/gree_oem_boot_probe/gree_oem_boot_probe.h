...
class GreeOemBootProbe : public Component {
 public:
  ...
  void set_query_energy_flow(bool enabled) { this->query_energy_flow_ = enabled; }
  void set_force_electrical_page_discovery(bool enabled) {
    this->force_electrical_page_discovery_ = enabled;
  }
  void set_force_energy_flow_discovery(bool enabled) {
    this->force_energy_flow_discovery_ = enabled;
  }
  void set_energy_flow_query_result_sensor(text_sensor::TextSensor *sensor) {
    this->energy_flow_query_result_sensor_ = sensor;
  }
  ...
  void setup() override {
    ...
    this->startup_sync_ = build_rtl_startup_sync(RTL_CLOUD_CONNECTED_MODULE_STATE);
    ...
    switch (this->phase_) {
      ...
      case Phase::BOOT_MAC_WAIT:
        if (this->consume_boot_information_44_()) {
          this->begin_rtl_startup_synchronization_(now);
        ...
        break;
      case Phase::BOOT_STARTUP_SYNC_1:
        this->send_and_advance_(this->startup_sync_, "RTL startup command 0x03 #1/4",
                                Phase::BOOT_STARTUP_SYNC_2, this->frame_spacing_ms_);
        break;
      case Phase::BOOT_STARTUP_SYNC_2:
        this->send_and_advance_(this->startup_sync_, "RTL startup command 0x03 #2/4",
                                Phase::BOOT_STARTUP_SYNC_3, this->frame_spacing_ms_);
        break;
      case Phase::BOOT_STARTUP_SYNC_3:
        this->send_and_advance_(this->startup_sync_, "RTL startup command 0x03 #3/4",
                                Phase::BOOT_STARTUP_SYNC_4, this->frame_spacing_ms_);
        break;
      case Phase::BOOT_STARTUP_SYNC_4:
        this->send_(this->startup_sync_, "RTL startup command 0x03 #4/4");
      ...
      case Phase::QUERY_ENERGY:
        if (this->should_query_energy_()) {
          ...
          this->send_query_and_advance_(QUERY_ENERGY_MONTH, 0x40,
                                        ...
                                        Phase::QUERY_ENERGY_FLOW);
        } else {
          this->finish_pending_query_();
          ESP_LOGI(TAG, "Skipping 0x40: 0x32 does not advertise ElcEn");
          this->phase_ = Phase::QUERY_ENERGY_FLOW;
          this->next_action_at_ = now;
        }
        break;
      case Phase::QUERY_ENERGY_FLOW:
        if (this->should_query_energy_flow_()) {
          ...
          this->send_query_and_advance_(
              QUERY_ENERGY_FLOW_FRAME, 0x53,
              advertised ? "firmware-advertised EnergyFlow poll command 0x09 -> 0x53"
                         : (this->energy_flow_response_seen_
                                ? "previously confirmed EnergyFlow refresh command 0x09 -> 0x53"
                                : "forced one-shot EnergyFlow discovery command 0x09 -> 0x53"),
              next);
        } else {
          this->finish_pending_query_();
          if (!this->query_energy_flow_) {
            ESP_LOGI(TAG, "Skipping 0x53 EnergyFlow poll: disabled by configuration");
            this->publish_energy_flow_query_result_("disabled");
          } else {
            ESP_LOGI(TAG, "Skipping 0x53 EnergyFlow poll: %s",
                     this->energy_flow_skip_reason_());
  ...
  void dump_config() override {
    ...
    ESP_LOGCONFIG(TAG, "  Startup frame spacing: %u ms", this->frame_spacing_ms_);
    ESP_LOGCONFIG(TAG, "  RTL startup module state: %u (OEM cloud connected)",
                  RTL_CLOUD_CONNECTED_MODULE_STATE);
    ...
    ESP_LOGCONFIG(TAG, "  Firmware EnergyFlow command-0x09 query: %s",
                  YESNO(this->query_energy_flow_));
    ESP_LOGCONFIG(TAG, "  Force one-shot unadvertised 0x40 discovery: %s",
                  YESNO(this->force_electrical_page_discovery_));
    ESP_LOGCONFIG(TAG, "  Force one-shot unadvertised EnergyFlow discovery: %s",
                  YESNO(this->force_energy_flow_discovery_));
 ...
 protected:
  ...
  void begin_initial_sequence_() {
    ...
    this->energy_discovery_attempted_ = false;
    this->energy_flow_discovery_attempted_ = false;
    this->energy_flow_response_seen_ =
        this->climate_->get_retained_payload_generation(0x53) != 0;
    ...
    ESP_LOGI(TAG, "Starting audited RTL8720CF startup followed by report queries");
  }
  ...
  void begin_rtl_startup_synchronization_(uint32_t now) {
    // The V2 and V3 schedulers transmit the initialized 29-byte command-0x03
    // buffer four times after identification. The former 17-byte replay came
    // from a different adapter generation and skipped this RTL startup stage.
    this->phase_ = Phase::BOOT_STARTUP_SYNC_1;
    this->next_action_at_ = now;
  }
  ...
  bool should_query_energy_() const {
    ...
    const bool response_seen =
        this->climate_->get_retained_payload_generation(0x40) != 0;
    const auto *synchronization = this->climate_->get_retained_payload(0x32);
    const bool capability_known =
        synchronization != nullptr && !synchronization->empty();
    const bool advertised = capability_known &&
                            (((*synchronization)[0] & 0x01U) != 0);
    return should_query_rtl_electrical_page(
        capability_known, advertised, response_seen,
        this->force_electrical_page_discovery_,
        this->energy_discovery_attempted_,
        this->query_cycle_ == QueryCycle::FULL_DISCOVERY);
  }

  EnergyFlowAdvertisement energy_flow_advertisement_() const {
    // Independent from the 0x40/ElcEn bits: both audited RTL schedulers gate
    // command 0x09 on command-0x32 payload[1] bit 1. A one-byte 0x32 payload
    // cannot advertise or deny that second-byte capability.
    const auto *synchronization = this->climate_->get_retained_payload(0x32);
    if (synchronization == nullptr || synchronization->empty()) {
      return EnergyFlowAdvertisement::NO_SYNCHRONIZATION;
    }
    if (synchronization->size() <= 1) {
      return EnergyFlowAdvertisement::CAPABILITY_BYTE_MISSING;
    }
    return ((*synchronization)[1] & 0x02U) != 0
               ? EnergyFlowAdvertisement::ADVERTISED
               : EnergyFlowAdvertisement::NOT_ADVERTISED;
  }

  bool energy_flow_advertised_() const {
    return this->energy_flow_advertisement_() ==
           EnergyFlowAdvertisement::ADVERTISED;
  }

  bool should_query_energy_flow_() const {
    const bool response_seen =
        this->energy_flow_response_seen_ ||
        this->climate_->get_retained_payload_generation(0x53) != 0;
    return should_query_rtl_energy_flow(
        this->query_energy_flow_, this->energy_flow_advertised_(),
        response_seen, this->force_energy_flow_discovery_,
        this->energy_flow_discovery_attempted_,
        this->query_cycle_ == QueryCycle::FULL_DISCOVERY);
  }

  const char *energy_flow_skip_state_() const {
    switch (this->energy_flow_advertisement_()) {
      case EnergyFlowAdvertisement::NO_SYNCHRONIZATION:
        return "no_0x32";
      case EnergyFlowAdvertisement::CAPABILITY_BYTE_MISSING:
        return "capability_byte_missing";
      case EnergyFlowAdvertisement::NOT_ADVERTISED:
        return "not_advertised";
      case EnergyFlowAdvertisement::ADVERTISED:
        return "advertised";
      default:
  ...
  const char *energy_flow_skip_reason_() const {
    switch (this->energy_flow_advertisement_()) {
      case EnergyFlowAdvertisement::NO_SYNCHRONIZATION:
        return "no 0x32 synchronization response";
      case EnergyFlowAdvertisement::CAPABILITY_BYTE_MISSING:
        return "0x32 payload[1] capability byte is missing";
      case EnergyFlowAdvertisement::NOT_ADVERTISED:
        return "0x32 payload[1] bit 1 is clear";
      case EnergyFlowAdvertisement::ADVERTISED:
        return "advertised";
      default:
  ...
  void publish_energy_flow_query_result_(const std::string &state) {
    if (this->energy_flow_query_result_sensor_ == nullptr ||
        state == this->last_energy_flow_query_result_) {
      return;
    }
    this->last_energy_flow_query_result_ = state;
    this->energy_flow_query_result_sensor_->publish_state(state);
  }
  ...
  void finish_pending_query_() {
    ...
    const uint32_t total_after =
        this->climate_->get_retained_payload_total_generation();
    const bool energy_flow_query = this->pending_expected_command_ == 0x53;
    ...
    } else if (total_after != this->pending_total_generation_) {
      const uint8_t command = this->climate_->get_last_retained_command();
      if (energy_flow_query) {
        char state[16];
        std::snprintf(state, sizeof(state), "other_0x%02X", command);
        this->publish_energy_flow_query_result_(state);
      }
      ESP_LOGW(TAG, "PROBE RX other response after %s: command 0x%02X; expected 0x%02X",
               this->pending_description_, command, this->pending_expected_command_);
    } else {
      if (energy_flow_query) {
        this->publish_energy_flow_query_result_("timeout");
      }
  ...
  void send_next_selector_discovery_query_() {
    ...
    std::snprintf(this->selector_description_, sizeof(this->selector_description_),
                  ...
                  primary, secondary);
    const auto frame = build_rtl_report_query(primary, secondary, 0x01);
  ...
  void send_query_and_advance_(const std::array<uint8_t, N> &frame, uint8_t expected_command,
    ...
    this->pending_total_generation_ =
        this->climate_->get_retained_payload_total_generation();
    this->pending_description_ = description;
    this->pending_query_active_ = true;
    this->send_(frame, description);
  ...
  void send_(const std::array<uint8_t, N> &frame, const char *description) {
    ...
    // and the 53-byte EnergyFlow frame for about 121 ms. The state-machine
    // response window makes a blocking flush unnecessary for either request.
    this->uart_->write_array(frame.data(), frame.size());
  }

  static constexpr const char *TAG = "gree.oem_boot_probe";
  // V2/V3 assign state 1 after a successful ACE/cloud server connection.
  // Emulating that state avoids advertising a partially connected OEM module.
  static constexpr uint8_t RTL_CLOUD_CONNECTED_MODULE_STATE = 0x01;

  // Command 0x02, full-MAC command 0x04, post-identification command 0x03,
  // and report selectors all come directly from the audited RTL8720CF V2/V3
  ...
  static constexpr auto QUERY_ENERGY_FLOW_FRAME = build_rtl_energy_flow_query();

  uart::UARTComponent *uart_{nullptr};
  sinclair_ac::SinclairAC *climate_{nullptr};
  text_sensor::TextSensor *energy_flow_query_result_sensor_{nullptr};
  ...
  bool query_energy_flow_{true};
  bool force_electrical_page_discovery_{false};
  bool force_energy_flow_discovery_{false};
  ...
  bool energy_flow_discovery_attempted_{false};
  bool energy_flow_response_seen_{false};
  std::string last_energy_flow_query_result_;
  std::array<uint8_t, 6> module_mac_{};
  std::array<uint8_t, RTL_MAC_REPORT_FRAME_SIZE> mac_report_{};
  std::array<uint8_t, RTL_REPORT_QUERY_SIZE> startup_sync_{};
